...
void print(char *name, double *a, int num){
        int i;
        printf("%s = %16f", name, a[0]);
        for (i = 1; i < num; i++)
                printf(",%s%16f",(i&3)?"":" ",a[i]);
        printf("\n");
}
double o[16*1024] = {0, };
double q[16*1024]={0, };
...
__m512d  simd1, simd2, simd3, simd4, simd5, simd6;
...
void init(struct network *net)
{
        ...
        //net->init_flag = (int *) malloc(sizeof(int) * net->mini_batch_size * TOTAL_NEURONS(net));
        ...
        net->u = (double *) malloc(sizeof(double) * net->mini_batch_size * TOTAL_NEURONS(net));
        // U(net, i+1, j, k) = ((rand())<p);
        ...
        for (i = 0; i < net->num_layer-1; i++)
        {
                ...
                for(j=0;j<net->mini_batch_size;j++)
                {
                        for (k = 0; k < net->layer_size[i+1]; k++)
                        {
                                ...
                                {
                                                simd2 = _mm512_load_ps(&NEURON(net, i, j, k));
                                                simd3 = _mm512_load_ps(&WEIGHT(net, i, l, k));
                                        //      sum = sum + NEURON(net, i, j, l) * WEIGHT(net, i, l, k);
                                                simd4 = _mm512_mul_ps(simd2, simd3);
                                                _mm512_store_ps(q,simd4);
                                               //sum *=net-> u[l];
                                }
                                ZS(net, i+1, j, k) = sum + BIAS(net, i+1, k);
                                //simd6 = _mm512_maskz_fmadd_ps(m16s, simd1, simd5, simd6);
                                NEURON(net, i+1, j, k) *= U(net, i+1, j, k);
        ...
        sum = 0.0;
                for (j = 0; j < net->num_layer-1; j++) {
                        ...
                        for (k = 0; k < net->layer_size[j+1]; k++) {
                                ...
                                ZS(net, j+1, 0, k) =( sum + BIAS(net, j+1, k))*p;
                                NEURON(net, j+1, 0, k) =U* sigmoid(ZS(net, j+1, 0, k));
                                sum = 0.0;
                        }
                }
